...
def get_groups():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT dashboard_groups.id, dashboard_groups.name, dashboard_faculties.name as faculty from dashboard_groups
                                LEFT JOIN dashboard_faculties ON dashboard_faculties.id = dashboard_groups.faculty_id""")
...
def get_subjects():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT * FROM dashboard_subjects""")
...
def get_teachers():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT dashboard_teachers.id, dashboard_teachers.first_name, dashboard_teachers.last_name, dashboard_teachers.age,
                    dashboard_chairs.name as chair_name, dashboard_subjects.name as subject_name FROM dashboard_teachers
...
def get_students():
    with closing(connection.cursor()) as cursor:
        cursor.execute("""SELECT dashboard_students.first_name, dashboard_students.last_name, dashboard_students.age,
                    dashboard_students.image as image, dashboard_groups.name as group_name FROM dashboard_students LEFT JOIN
                    dashboard_groups ON dashboard_groups.id = dashboard_students.group_id""")
